 ...
 * This code allows to execute write and read commands over serial monitor.
 * 
 * INSTRUCTIONS:
...
void onTotemRobot(TotemRobot robot) {
    Serial.print("Found: Name: \"");
    Serial.print(robot.getName());
    Serial.print("\", Module: ");
    Serial.print(robot.getNumber());
    Serial.print(", Color: #");
    Serial.print(robot.getColor(), HEX);
...
void onTotemRobotConnect(TotemRobot robot) {
...
void setup() {
    ...
    Serial.println("Looking for the module...");
    Totem.BLE.attachOnConnection(onTotemRobotConnect);
    Totem.BLE.findRobot(onTotemRobot);
    // Proceed to loop
}
...
Result processAPICall(ApiCall &call) {
    if (strcmp(call.function, "write") == 0 || strcmp(call.function, "w") == 0) {
        ...
        if (call.paramCount == 0) {
            if (!module.writeWait(call.command)) return ParseFailExec;
            return ParseOk;
        }
            ...
            switch (call.param[0].type) {
                ...
                case Param::Type::Char:
                    if (!module.writeWait(call.command, *call.param[0].getByte())) return ParseFailExec;
                    break;
                case Param::Type::Int:
                    if (!module.writeWait(call.command, *call.param[0].getInt())) return ParseFailExec;
                    break;
                case Param::Type::String:
                    if (!module.writeWait(call.command, (uint8_t*)call.param[0].getString(), strlen(call.param[0].getString()))) return ParseFailExec;
                    break;
            ...
            if (!module.writeWait(call.command, *call.param[0].getInt(), *call.param[1].getInt(), *call.param[2].getInt())) return ParseFailExec;
            return ParseOk;
        }
        else if (call.paramCount == 4) {
            if (module.writeWait(call.command, *call.param[0].getInt(), *call.param[1].getInt(), *call.param[2].getInt(), *call.param[3].getInt()))  return ParseFailExec;
            return ParseOk;
        }
    }
        ...
        if (!module.readWait(call.command, call.result)) {
        ...
        if (!module.subscribeWait(commandHash, *call.param[0].getInt())) {
            return ParseFailExec;
        }
        for (int i=0; i<(sizeof(subscribedCmd)/sizeof(SubscribedCmd)); i++) {
        ...
        uint32_t commandHash = TotemModule::hashCmd(call.command);
        if (!module.unsubscribeWait(commandHash)) {
            return ParseFailExec;
        }
        for (int i=0; i<(sizeof(subscribedCmd)/sizeof(SubscribedCmd)); i++) {
