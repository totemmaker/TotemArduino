...
/*
  PS3 Gamepad ====> (Bluetooth) ====> ESP32 board ====> (BLE) ====> Mini Control Board (MiniTrooper)
  Control MiniTrooper trough external ESP32 board (https://totemmaker.net/product/mini-trooper/)

  NOTE: If you have RoboBoard X3, connect PS3 Gamepad directly. Follow this example:
  https://github.com/totemmaker/arduino-examples/blob/master/kit_MiniTrooper/PS3_Controller/PS3_Controller.ino
...
void setup() {
  ...
  Serial.println("Waiting for PS3 controller to connect...");
...
void loop() {
  ...
  if (!Ps3.isConnected() || !board.isConnected()) { ESP.restart(); }
  /////////////////////////////////
  // Read state of controller buttons and axis
  /////////////////////////////////
  int flipPos = map(max(Ps3.data.analog.button.l2, Ps3.data.analog.button.r2), 0, 255, -100, 100);
  int drive = map(-Ps3.data.analog.stick.ly, -128, 127, -100, 100);
  int turn = map(Ps3.data.analog.stick.rx, -128, 127, -100, 100);
  if (abs(drive) < 5) drive = 0;
